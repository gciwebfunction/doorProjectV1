...
class CartController extends Controller
{
    ...
    public function addDoorToCart($productId, $cartId)
    {
        ...
        if (isset($data['handle_color_select']))
            $handleColorOption = Product\Door\AdditionalOption::find($data['glass_grid_select']);

        if (isset($data['lock_color_select']))
            $lockColorOption = Product\Door\AdditionalOption::find($data['blind_option_select']);

        if (isset($data['sill_color_select']))
            $sillColorOption = Product\Door\AdditionalOption::find($data['lite_option_select']);
...
//            $this->saveDoorItemModifier($doorItem->id, 'GLASS_DEPTH', $glassDepth->name ?? '', false, $glassDepth->multiplier, $glassDepth->price ?? 0);
    ...
    private
    function saveCartItem(Product $product, ShoppingCart $shoppingCart, ProductOption $productOption): CartItem
    {
    ...
    public function addDoorToCart_BKKK($productId, $cartId)
    {
        $door = Product\Door\Door::findOrFail($productId);
        $doorPrice = 0;
        $data = '';
