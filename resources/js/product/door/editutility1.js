...
function getSourceForHandlingInput(handlingCount) {
    return '<input class="form-control  dataField" type="text" size="70" ' +
        'name="doorhandling-' + handlingCount + '" id="doorhandling-' + handlingCount + '" ' +
        'placeholder="Door Handling" value="">' +
        '<p class="ml-10 handlingDelete" style="text-transform: capitalize;font-size: small;font-weight: bold;color: red;' +
        ' cursor:pointer;" id="doorhandling-delete-' + handlingCount + '">Delete</p>';
}

function getSourceForMeasurementInput(measurementCount) {
    return '<input class="form-control-sm dataField" type="text" name="width-' + measurementCount + '" ' +
        'placeholder="Width" id="doorWidthMeasurement-' + measurementCount + '" >' +
        '<input class="form-control-sm dataField" type="text" name="height-' + measurementCount + '" ' +
        'placeholder="Height" id="doorHeightMeasurement-' + measurementCount + '">' +
        '<p class=" ml-10 measurementDelete" style="text-transform: capitalize;font-size: small;font-weight: bold;color: red; ' +
        'cursor:pointer;" id="doorMeasurements-delete-' + measurementCount + '-0">Delete</p>';
}

function getSourceForFrame(frameCount) {
    return '<input class="form-control-sm dataField doorFrameClass" size="70" type="text" ' +
        'name="frame-' + frameCount + '" ' +
        'id="frame-' + frameCount + '" ' +
        'placeholder="Frame Option" value="">' +
        '<p class=" ml-10 doorFrameDelete" ' +
        'style="text-transform: capitalize;font-size: small;' +
        'font-weight: bold;color: red;cursor:pointer;" ' +
        'id="doorFrame-delete-' + frameCount + '">' +
        'Delete</p>';
}

function getSourceForColor(colorCount) {
    return '<input class="form-control-sm dataField colorClass" size="70" type="text" ' +
        'name="color-' + colorCount + '" ' +
        'id="color-' + colorCount + '" ' +
        'placeholder="Finish/Color" value=""> ' +
        '<p class=" ml-10 colorDelete" ' +
        'style="text-transform: capitalize;font-size: small;font-weight: bold;color: red; ' +
        'cursor:pointer;" ' +
        'id="doorColor-delete-' + colorCount + '">' +
        'Delete</p>';
}
    ...
    $('.handlingDelete').each(function () {
        this.addEventListener('click', function (event) {
            ...
            try {
                ...
                let handlingcount = $('#doorHandlingCount').val();
    ...
    $('.measurementDelete').each(function () {
        this.addEventListener('click', function (event) {
            ...
            try {
                ...
                let measurementCount = $('#doorMeasurementsCount').val();
    ...
    $('.colorDelete').each(function () {
        this.addEventListener('click', function (event) {
            ...
            try {
                ...
                counter = 0;
                $('.colorDelete').each(function () {
                    let colorDeleteIdArray = $(this).attr('id').split('-');
    ...
    $('.doorFrameDelete').each(function () {
        this.addEventListener('click', function (event) {
            ...
            try {
                ...
                let frameOptionCount = $('#frameOptionCount').val();
...
function init() {
    document.getElementById('categorySelection').addEventListener('change', function (event) {
        let selectedCategoryId = $('#categorySelection').val();
    ...
    document.getElementById('doorTypesList').addEventListener('change', function (event) {
        let subCatVal = $('#doorTypesList').val();
        if (subCatVal > 0) {
    ...
    if (isGliding == 0) {
        ...
        document.getElementById('addFrameOptionButton').addEventListener('click', function (event) {
            event.preventDefault();
            let frameCount = $('#frameOptionCount').val();
    ...
    document.getElementById('addSizeButton').addEventListener('click', function (event) {
        event.preventDefault();
        let doorMeasurementsCount = $('#doorMeasurementsCount').val();
...
function getSourceAdditionalTypeAndImageFields(index) {
    return '<div id="addOnDoorNameTypePlaceholder-' + index + '" style="padding: 3px; margin: 1px; border: 1px solid lightgray; ">' +
        '<div class="col-lg p-3 m-3" id="doorTypeContainer' + index + '">' +
        '<label class="form-label" for="doorNameType-0" id="doorNameTypeLabel-' + index + '">' +
        'Name or Type (ie.' + ' Inswing)' +
        '' +
        '</label>' +
        '<input type="text" id="doorNameType-' + index + '" name="door_name_type-' + index + '" ' +
        'class="form-control productFormElement dataField" style="min-width: 300px"/>' +
        '</div>' + '<div class="col-lg p-3 m-3" id="doorTypeImageContainer' + index + '">' +

        '<label class="form-label" for="doorNameType" id="doorNameTypeLabel">' + ' Add Image for Type' +
        '</label>' +
        '<input type="file" style="margin-left: 20px;" name="door_type_image-' + index + '" aria-describedby="doorTypeImage"' +
        ' id="door_type_image-' + index + '" class="form-control-file">' +
        '</div>' +
        '<div class="col-lg p-3 m-3">' +
        '<label class="form-label" for="doorNameType" id="doorNameTypeLabel">' + ' Remove this type...' +
        '</label>' +
        '   <span class="deleteProduct" style="cursor: pointer; text-align: center" ' +
        '   onclick="' +
        '   function e(){' +
        '                   $(\'#addOnDoorNameTypePlaceholder-' + index + '\').remove();' +
        '                   let count = +$(\'#typeCount\').val();' +
        '                   count--;' +
        '                   $(\'#typeCount\').val(count);' +
        '   } ' +
        'e()">X</span>' +
