...
/*!******************************************!*\
  !*** ./resources/js/product/utility1.js ***!
  \******************************************/
...
function init() {
  document.getElementById('categoriesDatalist').addEventListener('change', function (event) {
    var cat = $('#categoriesDatalist').val();
    ...
    $.get('/p/helper/door/' + cat, function (data) {
      $('#errorContainer').text("");
      $.each(data, function (index, element) {
        $('#doorTypeListContainer').removeClass('d-none');

        if (element.door_type) {
          $('#doorTypesDatalist').append('<option class="doortypeoption" value="' + element.id + ' - ' + element.door_type_pretty_name + '">' + element.door_type_pretty_name + '</option>');
        }
      });
