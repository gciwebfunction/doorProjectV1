...
function getCategoryType(selectedCategoryId) {
    let type = $('#categoryTypeHolder-' + selectedCategoryId).val()
...
function categorySelectionUpdate() {
    ...
    let categoryType = getCategoryType(selectedCategoryId);
...
function init() {
    ...
    document.getElementById('addSpecifierInputField').addEventListener('click', function (event) {
        event.preventDefault();
        let count = +($('#optionalSelectCount').val());
...
function getSourceAdditionalTypeAndImageFields(index) {
    return '<div class="col-lg p-3 m-3" id="doorTypeContainer' + index + '">' +
        '<label class="form-label" for="doorNameType-0" id="doorNameTypeLabel-' + index + '">Name or Type (ie.' +
        ' Inswing)</label>' +
        '<input type="text" id="doorNameType-' + index + '" name="door_name_type-' + index + '" ' +
        'class="form-control productFormElement dataFieldDoorProduct" style="min-width: 300px"/>' +
        '</div>' +
        '<div class="col-lg p-3 m-3" id="doorTypeImageContainer' + index + '">' +
        '<label class="form-label" for="doorNameType" id="doorNameTypeLabel">' +
        ' Add Image for Type </label>' +
        ...
        '<p><span id="doorNameTypeSpan-' + index + '" class="deleteProduct" style="cursor: pointer" onclick="function f(){' +
        '$(\'#doorTypeContainer' + index + '\').remove();' +
        '$(\'#doorTypeImageContainer' + index + '\').remove();' +
        'let count = +$(\'#typeCount\').val();' +
        'count--;' +
        '$(\'#typeCount\').val(count);' +
        '} f()">Remove this type</span></p>' +
        '</div>';
...
function getSourceForAdditionalSpecifier(count) {
    return '<div class="row p-3 m-3" id="additionalSpecifierNewRow-' + count + '">\n' +
        '        <div class="col"><label class="form-label" for="additionalDoorSpec-' + count + '" id="subCategoryNameLabel-' + count +
        '">Optional Select (ie. Full Lite)...</label></div>\n' +
        '        <div class="col">' +
        '<input type="text" id="additionalDoorSpec-' + count + '" name="additional_door_spec-' + count + '">' +
        '<span id="additionalDoorSpecSpan-' + count + '" class="deleteProduct" style="cursor: pointer" onclick="function f(){' +
        '$(\'#subCategoryNameLabel-' + count + '\').remove();' +
        '$(\'#additionalSpecifierNewRow-' + count + '\').remove();' +
        '$(\'#additionalDoorSpec-' + count + '\').remove();' +
        '$(\'#additionalDoorSpecSpan-' + count + '\').remove();' +
        'let count = +$(\'#optionalSelectCount\').val();' +
        'count--;' +
        '$(\'#optionalSelectCount\').val(count);' +
        '} f()"> X </span></div></div>';
...
function formIsValid() {
    let validation = '';
    let categoryType = getCategoryType($('#categorySelection').val());
    let classType = '';
    switch (categoryType) {
        case 'DOORS':
    ...
    $(classType).each(function () {
        if ($(this).val().length < 1)
            validation = validation + ' You must supply a name for all name/type fields.';
    })
